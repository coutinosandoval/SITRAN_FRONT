...
export class VehiculoComponent implements OnInit {
  ...
  vehiculoTraslado: Vehiculo | null = null;
  idSedeDestinoTraslado: number = 0;
  ...
  estadosDisponibilidad = ['Disponible', 'No Disponible', 'En Comision', 'En Mantenimiento'];
  ...
  guardar(): void {
    ...
    if (this.modoEdicion && this.vehiculoSeleccionado) {
      ...
      this.vehiculoService.agregar(datos).subscribe({
        next: () => {
          this.cargando = false;
  ...
  // Abre el modal de traslado
  abrirTraslado(vehiculo: Vehiculo): void {
    this.vehiculoTraslado = vehiculo;
    this.idSedeDestinoTraslado = 0;
  ...
  cerrarTraslado(): void {
    ...
    this.vehiculoTraslado = null;
  }

  // Guarda el traslado del vehículo a la sede seleccionada
  guardarTraslado(): void {
    if (!this.vehiculoTraslado) return;

    if (!this.idSedeDestinoTraslado) {
      alert('Seleccione la sede destino.');
      return;
    }

    this.vehiculoService
      .trasladar(this.vehiculoTraslado.id, this.idSedeDestinoTraslado, this.observacionesTraslado)
      .subscribe({
        next: () => {
          alert('Vehículo trasladado correctamente.');
          this.cerrarTraslado();
          this.cargarVehiculos();
        },
        error: (err) => {
          alert(err.error?.mensaje || 'Error al trasladar el vehículo.');
        },
      });
  }
}
